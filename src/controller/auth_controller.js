...
export const verifyToken = (req, res, next) => {
  ...
  if (typeof bearerHeader !== 'undefined') {
    ...
    jwt.verify(req.token, 'secretkey', (err, authData) => {
      if (err) {
        res.send({
          message: "Un authorized access, your token is not valid!!!"
        });
      } else {
        next();
      }
...
export const checkDuplicate = (req, res, next) => {
  Users.findOne({
    ...
    .then(usr => {
      if (usr) {
        res.send({
          message: 'This email is registered with an other account'
        })
      } else {
        next();
      }
    ...
    jwt.verify(req.token, 'secretkey', (err, tokenData) => {
      if (err) {
        ...
        Users.findOne({
          ...
          .exec()
          .then(usr => {
            if (usr.role !== 'admin') return res.status(401).send({
              auth: false,
              message: "Unable to perfom this action, You are not admin!"
            });
            next();
          }).catch(
            err => {
